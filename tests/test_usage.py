...
def test_hit_rate_counts_cold_start_as_a_miss():
    ...
    assert (
        CacheUsage(
            uncached_tokens=0, cached_tokens=0, write_tokens=9_000, output_tokens=0
        ).hit_rate()
        == 0.0
    )
...
def test_hit_rate_empty_prompt_does_not_divide_by_zero():
    assert (
        CacheUsage(uncached_tokens=0, cached_tokens=0, write_tokens=0, output_tokens=0).hit_rate()
        == 0.0
    )
...
def test_cost_usd_prices_each_slice_at_its_own_rate():
    ...
    usage = CacheUsage(
        uncached_tokens=1_000_000, cached_tokens=1_000_000, write_tokens=1_000_000, output_tokens=0
    )
